# מסמך עלboarding - MyTurn (סידור תורנויות)

## תוכן עניינים
1. [הקדמה ורעיון](#הקדמה-ורעיון)
...
**MyTurn** (סידור תורנויות) הוא אפליקציית ווב לניהול וסידור תורנויות עבור ארגונים הדורשים ניהול מורכב של משמרות, תפקידים ואנשי צוות.

### הרעיון המרכזי
המערכת נועדה לפשט את תהליך יצירת לוח זמנים מורכב עבור ארגונים הדורשים:
- ניהול מספר רב של אנשים ותפקידים
- שמירה על כללי מנוחה בין משמרות
- התחשבות בהעדפות אישיות ומגבלות
- ניהול קבוצות מיוחדות (כ"כ - כיתת כוננות)
...
- **ולידציה בזמן אמת** - בדיקת כללים ומגבלות לפני שמירה
- **גמישות** - אפשרות לעריכה ידנית לאחר יצירה אוטומטית
- **שקיפות** - תצוגה ברורה של חלוקת העבודה והצדק
...
- **הוספה/מחיקה** של אנשים עם פרטים:
  ...
  - שמירה כפולה (duelGuard)
  - פטור משמירה לילית (nightGuardExemption)
  - פטור מאסטמה (asthmaExemption) - מאפשר שיבוץ רק בתפקיד "תצפיתן"
  - פטור ממטבח (kitchenExemption)

- **ייבוא מאקסל** - ייבוא אנשים מקובץ Excel עם תמיכה בעמודות: name, gender, sameGenderPref, limitedAbility

### 2. ניהול תפקידים (Posts Management)
- יצירה ועריכה של תפקידים/עמדות
- הגדרת מספר אנשים נדרש לכל משמרת
- תפקידים אופציונליים (optional)

### 3. יצירת לוח זמנים (Schedule Generation)

#### סוגי לוחות זמנים:
- **לוח זמנים כללי** - כולל שמירות, BW, מטבח, ליווי, רס"ר, ליווי 400
- **לוח זמנים לשמירות בלבד** - יצירה נפרדת לשמירות
- **לוח זמנים למטבח** - יצירה נפרדת למטבח וליווי
- **לוח זמנים לרס"ר** - יצירה נפרדת לרס"ר וליווי 400
...
- **מגבלות** - התחשבות במגבלות אישיות (פטורים, העדפות)
...
#### ליווי 400 (Escort 400)
- 2 משמרות קבועות:
  - ליווי 400 1: 08:00-12:30
  - ליווי 400 2: 12:30-17:00
- כלל: רק נשים (F) יכולות להיות משובצות
- תמיכה ב-overrides למשמרות ספציפיות
...
- עריכה של קבוצות כ"כ
- עריכה של משמרות BW
- עריכה של משמרות מטבח וליווי
- עריכה של משמרות רס"ר וליווי 400
- ולידציה בזמן אמת בעת עריכה

### 7. מגבלות (Constraints)
- הוספת מגבלות זמן לאנשים ספציפיים
- המגבלות נחשבות בעת יצירת לוח זמנים
- מניעת שיבוץ במשמרות חופפות למגבלות
...
│   │   │   │   └── index.ts
│   │   │   ├── ConstraintsEditor.tsx         # עריכת מגבלות
...
- `GET /api/constraints` - רשימת כל המגבלות
