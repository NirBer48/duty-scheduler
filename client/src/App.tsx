...
const App: React.FC = () => {
  ...
  const handleManpowerConfirm = async () => {
    ...
    // User confirmed - they understand there's not enough manpower
    // Set the error message so they know the schedule wasn't created
    setError(t('not enough manpower'));
    setMissingCount(manpowerShortage);
  };
    ...
    <Box sx={{ minHeight: '100vh', bgcolor: '#f3f4f6' }}>
      ...
      >
        ...
        <DialogActions sx={{ px: 3, pb: 3, gap: 1, justifyContent: 'center' }}>
          <Button 
            onClick={() => setManpowerDialogOpen(false)}
            variant="outlined"
            sx={{ 
              px: 4, 
              borderRadius: 2,
